...
namespace Lof\SocialLogin\Controller\Stackoverflow;
...
use Lof\SocialLogin\Model\Stackoverflow as Stackoverflow;
use Lof\SocialLogin\Helper\Stackoverflow\Data as SocialHelper;
use Lof\SocialLogin\Model\SocialFactory as StackoverflowModelFactory;
use Lof\SocialLogin\Model\ResourceModel\Social\CollectionFactory as StackoverflowCollectionFactory;
use Magento\Customer\Api\CustomerRepositoryInterface;
use Magento\Customer\Model\Visitor;

class Callback extends Action
{
    const SOCIAL_TYPE = 'stackoverflow';
    ...
    protected $session;
    protected $stackoverflowCustomerCollectionFactory;
    protected $stackoverflowCustomerModelFactory;
    ...
    public function __construct(
        ...
        StackoverflowModelFactory $stackoverflowCustomerModelFactory,
        StackoverflowCollectionFactory $stackoverflowCustomerCollectionFactory,
        ...
        $this->stackoverflowCustomerModelFactory      = $stackoverflowCustomerModelFactory;
        $this->stackoverflowCustomerCollectionFactory = $stackoverflowCustomerCollectionFactory;
    ...
    public function execute()
    {
        ...
        if ($_GET['state'] !== $_SESSION['wpcc_state_stackoverflow']) {
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, "https://stackexchange.com/oauth/access_token");
        ...
        $response = explode('&', $auth);
        $accesstoken = explode('=', $response[0]);
        $curl = curl_init('https://api.stackexchange.com/2.2/me?key='.$key.'&site=stackoverflow&order=desc&sort=reputation&access_token='.$accesstoken[1].'&filter=default');
        curl_setopt($curl, CURLOPT_POST, false);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_ENCODING, "gzip");
        //curl_setopt( $curl, CURLOPT_HTTPHEADER, array( 'Accept-Encoding: gzip'));
        $dataResponse = json_decode(curl_exec($curl));
        $dataUser = (array)$dataResponse->items[0];
        $data = [];
    ...
    public function getCustomerIdByStackoverflowId($stackoverflowId)
    {
        $customer = $this->stackoverflowCustomerCollectionFactory->create();
        $dataUser     = $customer
        ->addFieldToFilter('social_id', $stackoverflowId)
    ...
    public function setAuthorCustomer($stackoverflowId, $customerId, $username)
    {
        $stackoverflowCustomer = $this->stackoverflowCustomerModelFactory->create();
        $stackoverflowCustomer->setData('social_id', $stackoverflowId);
        $stackoverflowCustomer->setData('username', $username);
        $stackoverflowCustomer->setData('customer_id', $customerId);
        $stackoverflowCustomer->setData('type', self::SOCIAL_TYPE);
        $stackoverflowCustomer->setData('is_send_password_email', $this->socialHelper->sendPassword());
        try {
            $stackoverflowCustomer->save();
        } catch (Exception $e) {
            $this->messageManager->addError(__('We can\'t process your request right now. Sorry, that\'s all we know.'));
        }
        return;
