...
namespace Lof\SocialLogin\Controller\Weibo;
...
use Lof\SocialLogin\Model\SocialFactory as WeiboModelFactory;
use Lof\SocialLogin\Model\ResourceModel\Social\CollectionFactory as WeiboCollectionFactory;
use Lof\SocialLogin\Helper\Weibo\Data as SocialHelper;
use Lof\SocialLogin\Model\Weibo;
use Magento\Customer\Api\CustomerRepositoryInterface;
use Magento\Customer\Model\Visitor;

class Callback extends Action
{
    const SOCIAL_TYPE = 'weibo';
    ...
    protected $session;
    protected $weiboCustomerCollectionFactory;
    protected $weiboCustomerModelFactory;
    ...
    public function __construct(
        ...
        WeiboModelFactory $weiboCustomerModelFactory,
        WeiboCollectionFactory $weiboCustomerCollectionFactory,
        ...
        $this->weiboCustomerModelFactory      = $weiboCustomerModelFactory;
        $this->weiboCustomerCollectionFactory = $weiboCustomerCollectionFactory;
    ...
    { 
        ...
        if ($state !== $_SESSION['wpcc_state_weibo']) {
    ...
    public function getCustomerIdByWeiboId($weiboId)
    {
        $customer = $this->weiboCustomerCollectionFactory->create();
        $dataUser     = $customer
        ->addFieldToFilter('social_id', $weiboId)
    ...
    public function setAuthorCustomer($weiboId, $customerId, $username)
    {
        $weiboCustomer = $this->weiboCustomerModelFactory->create();
        $weiboCustomer->setData('social_id', $weiboId);
        $weiboCustomer->setData('username', $username);
        $weiboCustomer->setData('customer_id', $customerId);
        $weiboCustomer->setData('type', self::SOCIAL_TYPE);
        $weiboCustomer->setData('is_send_password_email', $this->socialHelper->sendPassword());
        try {
            $weiboCustomer->save();
        } catch (Exception $e) {
            $this->messageManager->addError(__('We can\'t process your request right now. Sorry, that\'s all we know.'));
